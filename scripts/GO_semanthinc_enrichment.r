...
in_dir    <- file.path(base_dir, "files/network_conservation/enrichment_conserved")
out_dir   <- file.path(in_dir, "semantic")

ontologies       <- c("BP", "MF", "CC")   # each analysed independently
...
analyse_ontology <- function(ont) {
  ...
  a$score_a <- score_from_padj(a$p.adj)
  b$score_b <- score_from_padj(b$p.adj)
  ...
  # --- table ---------------------------------------------------------------
  out_tab <- uni %>%
    mutate(p.adj_sugarcane = 10^(-score_a), p.adj_purple = 10^(-score_b)) %>%
    select(GO.ID, Term, cluster, rep_Term, in_sugarcane = in_a, in_purple = in_b,
           shared, p.adj_sugarcane, p.adj_purple, Dim1, Dim2) %>%
