...
cfg <- list(
  sugarcane = list(
    map   = file.path(outdir, "module20_map_sugarcane.tsv"),
    nodes = file.path(base, "files/sugarcane/network_sugarcane_node_metrics.tsv"),
    strip = "\\.v[0-9.]+$",
    mods  = file.path(base, "files/sugarcane/mcl_sugarcane_membership.tsv"),
    tf    = file.path(base, "GET_TFS/new/results/sugarcane/TF_in_network.tsv"),
    cons  = file.path(base, "files/network_conservation/conserved_genes_sugarcane_FULL.txt")),
  purple = list(
    map   = file.path(outdir, "module20_map_purple.tsv"),
    nodes = file.path(base, "files/purple/new/network_purple_node_metrics.tsv"),
    strip = NULL,
    mods  = file.path(base, "files/purple/new/mcl_purple_membership.tsv"),
    tf    = file.path(base, "GET_TFS/new/results/purple/TF_in_network.tsv"),
    cons  = file.path(base, "files/network_conservation/conserved_genes_purple_FULL.txt"))
)

members <- fread(file.path(outdir, "module20_members.tsv"))
