...
base_dir  <- "/dados04/jorge/comparative_saccharum"
ontology  <- "CC"          # "BP", "MF", or "CC"
fdr_threshold <- 0.05      # BH-corrected p-value cutoff
ntop      <- 20            # top terms in the dot plot
...
#   gene_list     = genes on >=1 conserved edge (step 11)  [interest set]
#   nodes         = node-metrics table; col 1 = every gene in the filtered
#                   network. The GO-annotated subset of these is the UNIVERSE
#                   (background), so enrichment tests what is special about the
#                   conserved genes RELATIVE TO THE NETWORK, not the genome.
#   node_id_strip = regex removed from node IDs to match the annotation /
#                   gene_list namespace. Sugarcane nodes carry a ".v2.1" suffix
#                   that the annotation lacks; purple needs no stripping (NULL).
networks <- list(
  list(
    label         = "sugarcane",
    annotation    = file.path(base_dir, "annotation/sugarcane/emapper.annotations"),
    gene_list     = file.path(base_dir, "files/network_conservation/conserved_genes_sugarcane_FULL.txt"),
    nodes         = file.path(base_dir, "files/sugarcane/network_sugarcane_node_metrics.tsv"),
    node_id_strip = "\\.v[0-9.]+$",
    out_dir       = file.path(base_dir, "files/network_conservation/enrichment_conserved/sugarcane")
  ),
  list(
    label         = "purple",
    annotation    = file.path(base_dir, "annotation/purple/emapper.annotations"),
    gene_list     = file.path(base_dir, "files/network_conservation/conserved_genes_purple_FULL.txt"),
    nodes         = file.path(base_dir, "files/purple/new/network_purple_node_metrics.tsv"),
    node_id_strip = NULL,
    out_dir       = file.path(base_dir, "files/network_conservation/enrichment_conserved/purple")
  )
)
...
#' Run topGO (classic Fisher + BH) and return a significant-terms data.frame.
run_topgo <- function(interesting_genes, gene2GO, geneUniverse, ontology) {
  ...
  table_all <- GenTable(
    GOdata, Classic = result_classic,
    topNodes = length(allGO), orderBy = "Classic"
  )
...
for (net in networks) {
  ...
  # --- enrichment ----------------------------------------------------------
  cat("  [4] Running topGO (classic Fisher + BH)...\n")
  res <- tryCatch(
...
if (length(summary_rows) > 0) {
  ...
  summary_file <- file.path(base_dir,
    "files/network_conservation/enrichment_conserved",
...
cmp_dir <- file.path(base_dir, "files/network_conservation/enrichment_conserved")
...
if (empty_terms(resA) || empty_terms(resB)) {
  ...
  a <- resA[match(shared_ids, resA$GO.ID), c("GO.ID", "Term", "Significant", "p.adj")]
  b <- resB[match(shared_ids, resB$GO.ID), c("Significant", "p.adj")]
  colnames(a) <- c("GO.ID", "Term", paste0("Significant_", labA), paste0("p.adj_", labA))
  colnames(b) <- c(paste0("Significant_", labB), paste0("p.adj_", labB))
  shared_df <- cbind(a, b)
  shared_df <- shared_df[order(pmax(shared_df[[paste0("p.adj_", labA)]],
                                    shared_df[[paste0("p.adj_", labB)]])), ]
