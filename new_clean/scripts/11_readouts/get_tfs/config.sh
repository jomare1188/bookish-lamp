...
case "$SPECIES" in
  sugarcane)
    ...
    NODE_METRICS="/dados04/jorge/comparative_saccharum/new_clean/results/sugarcane/network_sugarcane_node_metrics.tsv"
  ...
  purple)
    ...
    NODE_METRICS="/dados04/jorge/comparative_saccharum/new_clean/results/purple/network_purple_node_metrics.tsv"
...
# --- cache vs output --------------------------------------------------------
# CACHE_DIR is the ORIGINAL GET_TFS tree, read-only here. Steps 01-03 are
# sequence-only (hmmsearch against the proteome) and are network-independent, so
# their output is reused rather than recomputed -- 1-2 h per species saved.
# OUTDIR is under new_clean/, so a re-run never overwrites the previous results.
CACHE_DIR="${BASE}/results/${SPECIES}"
OUTDIR="/dados04/jorge/comparative_saccharum/new_clean/results/readouts/get_tfs/${SPECIES}"
mkdir -p "$OUTDIR"
# chunks/domtbl belong to the cached hmmsearch, not to this run
CHUNKDIR="${CACHE_DIR}/chunks"
DOMTBLDIR="${CACHE_DIR}/domtbl"
...
export SPECIES GETTFS_DIR HMM_DB RULES ASSIGN_PL BASE \
       ...
       OUTDIR CACHE_DIR CHUNKDIR DOMTBLDIR
